...
def generate_normal_transaction():
    return {
        "transaction_amount": np.random.normal(loc=100, scale=50),  # Avg $100 transaction
        "transaction_time": np.random.choice(time_of_day, p=[0.5, 0.4, 0.1]),  # Most during day
...
def generate_fraud_transaction():
    return {
        ...
        "transaction_time": np.random.choice(time_of_day, p=[0.1, 0.2, 0.7]),  # Most at night
        "transaction_type": np.random.choice(transaction_types),
